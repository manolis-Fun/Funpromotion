...
const endpoint =
  process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT || "https://react.woth.gr/graphql";
...
const mockProducts = [
  {
    ...
    description:
      "High-quality copper vacuum insulated tumbler perfect for hot and cold beverages.",
    variations: null,
  },
  {
    id: "2",
    ...
    description:
      "Eco-friendly backpack made from recycled materials, perfect for everyday use and travel.",
    variations: null,
  },
];
...
export const getPriceMarkups = gql`
  query {
    markupsFlat {
      key
      value
...
export const getPriceMultipliers = gql`
  query priceMultipliersFlat {
    multipliersFlat {
      key
      value
    }
...
`;
export const getQuantityDefaults = gql`
  query quantityDefaults {
    quantityDefaultsFlat {
      key
      value
    }
  }
`;
export const getShippingCosts = gql`
query shippingCost {
  shippingCostsFlat {
    key
    value
...
`
export const getShippingDays = gql `
query shippingDays {
  shippingDaysFlat {
    key
    value
...
export async function fetchAllProducts(maxItems = 1000) {
  ...
  try {
    ...
    while (hasNextPage && attempts < maxAttempts && all.length < maxItems) {
      try {
        const data = await graphqlClient
          ...
          .catch(() =>
            getMockData(getAllProductsPaginatedQuery, { first: 1000, after })
          );
    ...
    return getMockData(getAllProductsPaginatedQuery, { first: 1000 }).products
      .nodes;
  }
}
...
export const getMockData = (query, variables) => {
  ...
  if (queryString.includes("GetProductBySlug")) {
    const product =
      mockProducts.find((p) => p.slug === variables.slug) || mockProducts[0];
    return { product };
  }
