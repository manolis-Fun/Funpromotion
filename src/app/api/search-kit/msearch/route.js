    ...
    try {
      ...
      for (const request of requests) {
        ...
        const buildTermQuery = (field, value) => {
          ...
          // Default term query
          const fieldToUse = field.includes('.') && !field.endsWith('.keyword') ? `${field}.keyword` : field;
          return {
            term: { [fieldToUse]: value }
          };
        };
        ...
        if (params.filters && params.filters.trim()) {
          ...
          for (const filter of filterParts) {
            if (filter.includes(':')) {
              ...
              filters.push(buildTermQuery(cleanField, cleanValue));
            }
          }
        }
          ...
          for (const facetFilterGroup of params.facetFilters) {
            if (Array.isArray(facetFilterGroup)) {
              ...
              const orFilters = facetFilterGroup.map(facetFilter => {
                ...
                return buildTermQuery(field.trim(), value.trim());
              });
              ...
              attributeGroups[fieldKey].push(buildTermQuery(fieldKey, value.trim()));
            }
          }
